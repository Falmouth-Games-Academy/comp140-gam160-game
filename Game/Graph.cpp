...
void Graph::Render() const {
	// Draw title
	game.GetRenderer(RenderScreen::Debug).RenderText(title, renderX, renderY);
	...
	SDL_SetRenderDrawColor(game.GetSDLRenderer(RenderScreen::Debug), 0, 0, 0, 255);
	SDL_RenderDrawRect(game.GetSDLRenderer(RenderScreen::Debug), &border);

	// Draw graph
	SDL_SetRenderDrawColor(game.GetSDLRenderer(RenderScreen::Debug), clrR, clrG, clrB, 0);
	...
	SDL_RenderDrawLines(game.GetSDLRenderer(RenderScreen::Debug), points, numDrawnPoints);
	...
	for (const GraphBar& bar : bars) {
		...
		if (bar.orientation == GraphBar::Horizontal) {
			SDL_RenderDrawLine(game.GetSDLRenderer(RenderScreen::Debug), centreX - bar.length, centreY, centreX + bar.length, centreY);
		} else {
			SDL_RenderDrawLine(game.GetSDLRenderer(RenderScreen::Debug), centreX, centreY - bar.length, centreX, centreY + bar.length);
		}
	}
}